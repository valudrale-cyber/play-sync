  ...
  el.innerHTML = `
    ...
    <div class="track-foot">
      ...
      <div class="track-actions">
        <button class="btn tiny ghost" type="button" data-quote="1">Request quote</button>
...
/* =========================
   ...
   - Generates a pre-filled email draft via mailto: to Valentina.
   ========================= */

const QUOTE_TO_EMAIL = "valentina.draletti@daleplayrecords.com";
let quoteSelectedTrack = null;

function $(id){ return document.getElementById(id); }

function openQuoteModal(track){
  const modal = $("quote-modal");
  const subtitle = $("quote-subtitle");
  const form = $("quote-form");
  if (!modal || !subtitle || !form) return;

  quoteSelectedTrack = track;

  const title = track?.title ? `"${track.title}"` : "—";
  const artist = track?.artist ? track.artist : "—";
  subtitle.textContent = `Selected track: ${title} — ${artist}`;

  // Reset form + conditional blocks
  form.reset();
  $("rq-commercial")?.setAttribute("hidden", "hidden");
  $("rq-film")?.setAttribute("hidden", "hidden");

  modal.hidden = false;
  document.body.style.overflow = "hidden";
  // Focus first required
  setTimeout(() => $("rq-fullname")?.focus(), 0);
}
...
function requiredFilled(){
  const requiredIds = ["rq-fullname","rq-email","rq-phone","rq-company","rq-title","rq-type"];
...
function mailtoEncode(s){ return encodeURIComponent(String(s || "")); }

function buildEmailBody(data){
  const t = quoteSelectedTrack || {};
  const lines = [];

  lines.push("PLAY SYNC — Sync Quote Request");
  lines.push("");
  lines.push("TRACK");
  lines.push(`Title: ${t.title || "-"}`);
  lines.push(`Artist: ${t.artist || "-"}`);
  lines.push(`Genre: ${t.genre || "-"}`);
  lines.push(`ISRC: ${t.isrc || "-"}`);
  lines.push(`Rights: ${t.rights || "-"}`);
  lines.push(`Clearance: ${t.clearance || "-"}`);
  lines.push("");

  lines.push("REQUESTER (required)");
  lines.push(`Full name: ${data.fullname}`);
  lines.push(`Email: ${data.email}`);
  lines.push(`Phone: ${data.phone}`);
  lines.push(`Company: ${data.company}`);
  lines.push(`Role/Title: ${data.role}`);
  lines.push("");

  lines.push(`PROJECT TYPE: ${data.type}`);
  lines.push("");

  if (data.type === "COMMERCIAL/ADS" || data.type === "GAMING"){
    ...
    lines.push(data.territory || "-");
    lines.push("");
    lines.push("TERM:");
    lines.push(data.term || "-");
    lines.push("");
    lines.push("MUSIC BUDGET:");
    lines.push(data.musicbudget || "-");
  } else if (data.type === "FILM" || data.type === "SERIES"){
    lines.push("FILM / SERIES DETAILS");
    lines.push(`Licensee: ${data.licensee || "-"}`);
    lines.push(`Production Title: ${data.prodtitle || "-"}`);
    lines.push("");
    lines.push("Production Description/Synopsis:");
    lines.push(data.synopsis || "-");
    lines.push("");
    lines.push(`Rights/Media Requested: ${data.rightsmedia || "-"}`);
    lines.push(`Territory: ${data.territory2 || "-"}`);
    lines.push(`Term: ${data.term2 || "-"}`);
    lines.push("");
    lines.push("Scene Description:");
    lines.push(data.scene || "-");
    lines.push("");
    lines.push(`Type Of Use: ${data.typeuse || "-"}`);
    lines.push(`Duration Of Use: ${data.durationuse || "-"}`);
    lines.push(`Clearance Deadline: ${data.deadline || "-"}`);
    lines.push(`Exhibition/Start Date: ${data.startdate || "-"}`);
    lines.push("");
    lines.push(`Cast Members: ${data.cast || "-"}`);
    lines.push(`Film Director: ${data.director || "-"}`);
    lines.push(`Production Company: ${data.prodco || "-"}`);
    lines.push(`Music Budget: ${data.musicbudget2 || "-"}`);
    lines.push(`Overall Production Budget: ${data.overallbudget || "-"}`);
    lines.push(`Total Number Of Songs In Project: ${data.totaltracks || "-"}`);
    lines.push(`Other Artists/Songs In The Film: ${data.otherartists || "-"}`);
    lines.push(`License Fee Offered, Per Side: ${data.fee || "-"}`);
    lines.push(`Do You Have A Distributor?: ${data.distributor || "-"}`);
  }

  lines.push("");
  lines.push("Sent via PLAY SYNC trial form (mailto).");
  return lines.join("\n");
}
...
function initQuoteModal(){
  const modal = $("quote-modal");
  const form = $("quote-form");
  const typeSel = $("rq-type");
  if (!modal || !form || !typeSel) return;
  ...
  typeSel.addEventListener("change", updateQuoteSections);

  form.addEventListener("submit", (e) => {
    e.preventDefault();
    if (!requiredFilled()){
      alert("Please complete all required fields (name, email, phone, company, role and project type).");
      return;
    }
    ...
    const t = quoteSelectedTrack || {};
    const subject = `PLAY SYNC Quote Request — ${t.title || "Track"} / ${t.artist || ""} — ${data.type || ""}`.trim();
    const body = buildEmailBody(data);

    const href = `mailto:${QUOTE_TO_EMAIL}?subject=${mailtoEncode(subject)}&body=${mailtoEncode(body)}`;
